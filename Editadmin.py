...
admin_id = 1  # Replace with st.session_state.get("admin_id", 1) if needed
...
with st.form(key="edit_admin_form"):
    ...
    with col1:
        admin_firstname = st.text_input("First Name")
        admin_lastname = st.text_input("Last Name")
        admin_gender = st.selectbox("Gender", ["Male", "Female", "Others"])
        # admin_dateofbirth = st.date_input("Date of Birth")

    with col2:
        admin_phone = st.text_input("Phone Number")
        admin_email = st.text_input("Email")
        dmin_dateofbirth = st.date_input("Date of Birth")
    admin_address = st.text_area("Address")
